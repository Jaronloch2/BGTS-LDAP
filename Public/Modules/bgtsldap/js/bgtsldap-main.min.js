(function(){
	...
	.config(function($stateProvider, $urlRouterProvider) {
		$stateProvider
			...
			.state('bgtsldap-registerform', {
				...
				templateUrl: 'bgtsldap/user/create',
				data: {
			...
			.state('bgtsldap-user', {
				url: '/bgtsldap/user/{id}',
				templateUrl: function(stateParams){
					return 'bgtsldap/user/'+stateParams.id + '/?' + Math.random(12,189)
				},
				data: {
					requireLogin: true,
					redirectTo: "root" 
			...
			.state('bgtsldap-users', {
				url: '/bgtsldap/users',
				templateUrl: function(stateParams){
					return 'bgtsldap/user?' + Math.random(12,189)
				},
				data: {
					requireLogin: true,
					redirectTo: "root" 
			...
			.state('bgtsldap-user-edit', {
				url: '/bgtsldap/user/{id}/edit',
				templateUrl: function(stateParams){
					return 'bgtsldap/user/'+stateParams.id + '/edit/?' + Math.random(12,189)
				},
				data: {
					requireLogin: true,
					redirectTo: "root" 
	...
	.directive('file', function () {
		return {
			...
			link: function (scope, el, attrs) {
			  var onChangeFunc = scope.$eval(attrs.file);
	...
	.directive('bgtsLdapusers',[function(){
		return {
			restrict: 'E',
			templateUrl: 'bgtsldap/users'
		};
	}])
	...
	.service("BGTSUserService",['$http','$rootScope','$scope',function($http,$rootScope,$scope){
		var user	= this;
		user.getUser			= function(id){
			$http.get('bgtsldap/user/'+id).then(function(){
				
			});
		};
	}])
	.controller("UserController",["$http","$cookies","$scope","$rootScope","SyncSession","PopupModal",function($http,$cookies,$scope,$rootScope,SyncSession,PopupModal){
		var userCtrl				= this;
		userCtrl.user				= {};
		userCtrl.user._method		= 'POST';
		userCtrl.user.picture		= 'default.jpg';
		userCtrl.picture			= '/Modules/bgtsldap/images/users/pictures/' + userCtrl.user.picture;
		userCtrl.user._method		= "POST";
		$scope.$watch('userCtrl.user.picture',function(){
			userCtrl.user.picture		= userCtrl.user.picture===undefined ? 'default.jpg':userCtrl.user.picture;
			userCtrl.picture			= '/Modules/bgtsldap/images/users/pictures/' + userCtrl.user.picture;
		});
		userCtrl.popup			= function(title,msg){
			PopupModal.show(title,msg,true,true);
		}
		userCtrl.init			= function(user){
			if(user!==undefined){
				if(user.id!==undefined && user.id!==null && user.id!==''){
					userCtrl.user	= user;
					userCtrl.user._method		= "PATCH";
					userCtrl.user.password		= "ldappasswordlang";
					userCtrl.picture			= '/Modules/bgtsldap/images/users/pictures/' + userCtrl.user.picture;
				}		
			}			
		};
		userCtrl.user.password		= "ldappasswordlang";
		userCtrl.user.csrf_token	= angular.element('#csrf_token').val();
		userCtrl.uploadPicture		= function(event){
			console.log('Uploading Picture...');
			$http({
				method: 'POST',
				url: 'bgtsldap/uploadPicture',
				headers: {'Content-Type': undefined},
				transformRequest: function (data, headersGetter) {
					var fd 	= new FormData();
					fd.append('pic', event.target.files[0]);
					var headers = headersGetter();
					delete headers['Content-Type'];
					return fd;
				}
			}).then(function(r){
				$rootScope.oldPic		= userCtrl.user.picture;
				userCtrl.user.picture	= r.data.data;
			})
		}
		userCtrl.save			= function(){
			var id	= userCtrl.user.id!==undefined ? userCtrl.user.id:'';
			if($rootScope.oldPic!==userCtrl.user.picture){
				$http.post('bgtsldap/uploadPicture',{'del':$rootScope.oldPic});
			}
			$http.post("/bgtsldap/user/"+id,userCtrl.user).then(function(response){
				if(response.data.return){
					PopupModal.show(response.data.type,response.data.summary,true,true);
					if(id==''){
						userCtrl.user				= {};
						$scope.$close();
					}
